  ...
  struct TextSelectionModelTests {
    ...
    func urlForPoint() throws {
      guard #available(iOS 18, macOS 15, tvOS 18, watchOS 11, visionOS 2, *) else { return }
      // given
    ...
    @Test
    func urlAndBoundsForPoint() throws {
      guard #available(iOS 18, macOS 15, tvOS 18, watchOS 11, visionOS 2, *) else { return }
      // given: same fixture/points as `urlForPoint` — this is the lookup the text-selection
      // overlay (NSTextInteractionView/UITextInteractionView) uses to also fire
      // `textualEntityTapAction` on tap, alongside `openURL`, since selection intercepts touches
      // at the platform layer and bypasses TextLinkInteraction's SwiftUI gesture entirely.
      let model = try TextSelectionModel(fixtureName: "two-paragraphs-bidi")
      let hitPoint = CGPoint(x: 276, y: 25)
      let missPoint = CGPoint(x: 36, y: 25)
      let outsidePoint = CGPoint(x: 10, y: 10)
      ...
      // when
      let hit = model.urlAndBounds(for: hitPoint)
      let miss = model.urlAndBounds(for: missPoint)
      let outside = model.urlAndBounds(for: outsidePoint)

      // then
      #expect(hit?.url == URL(string: "https://example.com"))
      #expect(hit?.bounds.contains(hitPoint) == true)
      #expect(miss == nil)
      #expect(outside == nil)
    }
