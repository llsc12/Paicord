  ...
  final class UITextInteractionView: UIView {
    ...
    /// This view's own origin in SwiftUI's `.global` coordinate space, captured by a
    /// `GeometryReader` in `UIKitTextSelectionInteraction`. Added to purely-local tap points
    /// instead of using `UIView.convert(_:to: nil)`, which isn't guaranteed to land in exactly
    /// the same coordinate space SwiftUI's `.global` refers to.
    ...
    @objc private func handleTap(_ gesture: UITapGestureRecognizer) {
      ...
      guard let (url, bounds) = model.urlAndBounds(for: location) else {
