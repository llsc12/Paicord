  ...
  extension TextLayoutCollection {
    func url(for point: CGPoint) -> URL? {
      urlAndBounds(for: point)?.url
    }

    /// Like `url(for:)`, but also returns the tapped run's bounds, in the same coordinate space
    /// as `point` — useful for callers that want to anchor UI (for example, a popover) near the
    /// tapped entity, the same way `TextLinkInteraction`'s `textualEntityTapAction` hook does.
    func urlAndBounds(for point: CGPoint) -> (url: URL, bounds: CGRect)? {
      ...
      guard
        let run = layout.runs.first(where: { $0.typographicBounds.contains(localPoint) }),
        let url = run.url
      else {
        return nil
      }

      let bounds = run.typographicBounds.offsetBy(dx: layout.origin.x, dy: layout.origin.y)
      return (url, bounds)
    }
