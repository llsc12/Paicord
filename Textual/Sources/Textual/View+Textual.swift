...
extension TextualNamespace where Base: View {
  ...
  /// Registers an action to perform when a link-attributed run is tapped, in addition to the
  /// environment's `openURL` action.
  ///
  /// The action receives the tapped URL and the tapped run's bounds in the `.global` coordinate
  /// space — convert it into a local `UnitPoint` (for example with a `GeometryReader` tracking
  /// this view's own global frame) to anchor UI like a popover near the tapped entity, without
  /// reimplementing text hit-testing.
  ///
  /// `openURL` is always invoked too, so if you use a custom URL scheme for entity links (e.g.
  /// mentions), install an `OpenURLAction` that returns `.handled` for that scheme — otherwise the
  /// system will try (and fail) to open it externally.
  public func onEntityTap(_ action: @escaping @MainActor (URL, CGRect) -> Void) -> some View {
    base.environment(\.textualEntityTapAction, action)
  }
