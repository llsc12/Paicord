...
public struct InlineText: View {
  ...
  ///   - revision: Re-parses `markup` whenever this value changes, even if `markup` itself
  ///     doesn't — useful when your `parser` depends on external state (for example,
  ///     `syntaxExtensions` capturing a value that changed) that doesn't show up as a
  ///     difference in the source string. Updates the existing parsed content in place instead
  ///     of resetting the view's identity (as `.id(_:)` would), so the displayed content
  ///     doesn't momentarily collapse while re-parsing.
