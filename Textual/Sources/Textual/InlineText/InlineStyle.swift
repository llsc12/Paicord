...
public struct InlineStyle: Sendable, Hashable {
  ...
  var subtext: AnyTextProperty = AnyTextProperty(.fontScale(1))
  ...
  /// Returns a copy of this style with custom subtext formatting (for example, Discord's `-# `
  /// syntax — see ``TextualAttributes/SubtextAttribute``).
  ///
  /// - Parameter properties: Text properties to apply to subtext.
  /// - Returns: A new style with the modified subtext formatting.
  public func subtext<each P: TextProperty>(_ properties: repeat each P) -> Self {
    modifyingStyle { $0.subtext = AnyTextProperty(repeat each properties) }
  }
