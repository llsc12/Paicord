...
class MessageDrainStore: DiscordDataStore {
  ...
  var pendingMessages = [ChannelSnowflake: OrderedDictionary<MessageSnowflake, Payloads.CreateMessage>]()
  var failedMessages: [MessageSnowflake: Error?] = [:]
  
  var messageSendQueueTask: Task<Void, Never>?
  var messageTasks: [MessageSnowflake: @Sendable () async -> Void] = [:] {
    didSet {
      ...
      messageSendQueueTask = Task.detached { [weak self] in
        ...
        for (_, task) in self.messageTasks {
          do {
            ...
            // already handled in send method
          }
        }
      }
    }
  ...
  func send(_ message: Payloads.CreateMessage, in channel: ChannelSnowflake) {
    ...
    // set our message up
    let task: @Sendable () async -> Void = { [weak self] in
      guard let self else { return }
      do {
        try await gateway.client.createMessage(channelId: channel, payload: message)
          .guardSuccess()
      } catch {
        // mark as failed
    ...
    // store in pending
    pendingMessages[channel, default: .init()].updateValueAndMoveToFront(message, forKey: nonce)
  }
