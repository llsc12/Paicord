...
class CurrentUserStore: DiscordDataStore {
	...
	var guilds: [Guild] = []
	var privateChannels: [DiscordChannel] = []
	...
	private func handleGuildCreate(_ guild: Gateway.GuildCreate) {
			if let index = guilds.firstIndex(where: { $0.id == guild.id }) {
				guilds[index] = guild.toGuild()
			} else {
				guilds.append(guild.toGuild())
			}
	}
	
	private func handleGuildDelete(_ unavailableGuild: UnavailableGuild) {
		guilds.removeAll { $0.id == unavailableGuild.id }
	}
	...
	private func handleRelationshipUpdate(_ partialRelationship: Gateway.PartialRelationship) {
		if let index = relationships.firstIndex(where: { $0.id == partialRelationship.id }) {
	...
	private func handleRelationshipRemove(_ partialRelationship: Gateway.PartialRelationship) {
		relationships.removeAll { $0.id == partialRelationship.id }
	}
	
	private func handlePrivateChannelCreate(_ channel: DiscordChannel) {
		if let index = privateChannels.firstIndex(where: { $0.id == channel.id }) {
