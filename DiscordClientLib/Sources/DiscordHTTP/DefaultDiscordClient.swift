...
public struct DefaultDiscordClient: DiscordClient {
  ...
  public var captchaChallengeSolver: CaptchaChallengeHandler?
  ...
  public var userAgent: String {
    ...
    if case .userToken = self.authentication {
  ...
  func sendWithRetries(
    ...
    sendRequest: (CacheableEndpointIdentity?, Int, UInt) async throws -> (
    ...
    do {
      ...
      if let captchaData = extractCaptchaChallenge(from: response),
        let handler = self.captchaChallengeSolver,
        let captchaHeaderData: CaptchaSubmitData = await handler(captchaData)
      {
        headers.replaceOrAdd(
          name: "X-Captcha-Key", value: captchaHeaderData.solutionToken)
        #warning("add other headers for captcha solution")
      }
    ...
    -> DiscordHTTPResponse
  {
    try await self.sendWithRetries(request: req) {
	  ...
	  if case .userToken = authentication {
